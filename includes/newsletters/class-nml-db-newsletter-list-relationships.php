...
/**
 * Newsletter & List Relationships DB Class
 *
 * This class is for interacting with the list relationships database table.
 * Used for mapping relationships between lists and newsletters.
 *
...
/**
 * Class NML_DB_Newsletter_List_Relationships
 *
 * @since 1.0
 */
class NML_DB_Newsletter_List_Relationships extends NML_DB {

	/**
	 * NML_DB_Newsletter_List_Relationships constructor.
	 *
	 * @access public
	 * @since 1.0
	 * @return void
	...
	public function __construct() {
		...
		$this->table_name  = $wpdb->prefix . 'nml_newsletter_list_relationships';
		$this->primary_key = 'ID';
		$this->version     = '1.0';
	...
	public function add( $data = array() ) {
		...
		if ( $relationship ) {
			...
			return $this->insert( $args, 'newsletter_relationship' );
	 ...
	 * @return int|false The number of rows deleted, or false on failure.
	 */
	public function delete_newsletter_relationships( $newsletter_id ) {

		global $wpdb;

		$query = $wpdb->prepare( "DELETE FROM $this->table_name WHERE `newsletter_id` = %d", absint( $newsletter_id ) );
	...
	public function get_relationships( $args = array() ) {
		...
		$cache_key = md5( 'nml_newsletter_list_relationships_' . serialize( $args ) );
	...
	public function count( $args = array() ) {
		...
		$cache_key = md5( 'nml_newsletter_list_relationships_count_' . serialize( $args ) );
